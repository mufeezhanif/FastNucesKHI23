...
public:
    ...
    bool Search_Medicine(string _formula, string &Name, int &ind)
    {
        ...
        {
            cout << "loop called " << i+1 << endl;
            medicine[i].display();
            string temp=medicine[i].GetFormula();
            if (_formula == temp){
                cout << "condition true " << i << endl;
...
public:
    ...
    void Search_Medicine(Pharmicist &p)
    {
        ...
        cin >> _formula;
        if (p.Search_Medicine(_formula, medicineName, index) == 1)
        {
